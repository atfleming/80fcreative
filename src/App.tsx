...
const ShowreelTitle = lazy(() =>
	import("./components/ShowreelTitle").then((module) => ({
		default: module.ShowreelTitle
	}))
);
const AnimatedVideoSection = lazy(() =>
	import("./components/AnimatedVideoSection").then((module) => ({
		default: module.AnimatedVideoSection
	}))
);
		...
		<div className="bg-digital-black text-highlight min-h-screen w-full crt-scanlines">
			<div className="pixel-grid">
				<main>
					...
					{/* <Newsletter /> */}
					<Suspense fallback={<SectionFallback label="Contact call-to-action" className="mt-24" />}>
						<Contact />
					</Suspense>
				</main>
				<Suspense fallback={<SectionFallback label="Footer" className="mt-16" />}>
					<Footer />
				</Suspense>
			</div>
		</div>
